...
This is a modified version of the Node.js Chart provided by [Bitnami](https://bitnami.com/). The original README of which can be read below.

## Chart Modifications

A custom Node.js image running [Socket.IO](https://socket.io/) and new Node parameters have been added:
| Name | Description | Value |
| ------------------------- | ----------------------------------------------- | ----- |
| `extraEnvVars.ENABLE_REDIS_WORKER` | Enables communication with Redis Worker | `"true"` |
| `extraEnvVars.ENABLE_CELERY_WORKER` | Enables communication with Celery Worker | `"false"` |
| `extraEnvVars.ENABLE_BENTOML_WORKER` | Enables communication with [BentoML](https://github.com/bentoml/BentoML) Worker | `"false"` |
| `extraEnvVars.REDIS_URL` | URL of the Redis Worker | `"redis://myredis-headless"` |
| `extraEnvVars.CELERY_BROKER_URL` | URL of Celery's Broker | `"amqp://admin:mypass@rabbitmq-service:5672"` |
| `extraEnvVars.CELERY_RESULT_BACKEND` | URL of Celery's Backend | `"redis://myredis-headless:6379/0"` |
| `extraEnvVars.YATAI_DEPLOYMENT_URL` | URL of the [Yatai](https://github.com/bentoml/Yatai) deployment, aka the BentoML Worker | `"http://emotion-analyzer.yatai.svc.cluster.local:3000/predict_async"` |

Also, Ingress is enabled by default. The hostname is `kubernetesnode`. There are some additional annotations for a nginx Ingress resource in `ingress.annotations`.

## How it works

Clients connected to the [namespace](https://socket.io/docs/v4/namespaces/) `patients` [send](https://socket.io/docs/v4/emitting-events/) `image` events. The image is then forwarded to all enabled workers. The workers in turn recognize emotions and sent them back to Socket.IO.

Clients connected to the namespace `physicians` emit the `subscribe` event to receive patient results via the `emotion` event and send `unsubscribe` to cancel the subscription.

## Usage

Use [socket.io-client](https://socket.io/docs/v4/client-initialization/) to connect to the patient or physician namespace, e.g.

```javascript
const socket = io("https://kubernetesnode/physicians");
```

Emit the aforementioned events:

### Namespace _physicians_
...
**Receive emotion event**

```javascript
socket.on("emotion", function (msg) {
  const emotion = JSON.parse(msg);
  const userId = emotion.userId;
  const dominantEmotion = emotion.emotions[0].dominantEmotion;
});
```

### Namespace _patients_
